...
export default function Home({ onHide, transcriptionRecord }) {
  ...
  const autofillCB = (data) => {
    ...
    setTimeout(() => {
      autofillRef.current.scrollIntoView({
        behavior: "smooth",
        block: "center",
      });
    }, 1000);
    ...
    const shortlistPage = (template) => {
      if (template) {
        if (transcriptionRecord) {
          setPageData(
            template.pages.find(
              (page) => page.id === transcriptionRecord.page_id
            )
          );
        }
        ...
        {!pageData ? (
          pages === null ? (
            ...
            <div className="tw-flex tw-flex-col tw-gap-4 tw-justify-center tw-items-center tw-p-4">
              ...
              <span>
                You can setup {fqdn} as a WebApp at{" "}
                <a
                  href="https://app.medispeak.in"
                  target="_blank"
                  rel="noreferrer"
                  className="tw-text-blue-600 tw-underline"
                >
                  https://app.medispeak.in
                </a>
              </span>
            </div>
          ) : pages ? (
            ...
            pages.length === 0 ? (
              <div className="tw-flex tw-flex-col tw-gap-4 tw-justify-center tw-items-center tw-p-4">
                <span>No Pages found for this WebApp</span>
                <p>
                  You can setup <span className="tw-font-semibold">Pages</span>{" "}
                  for the WebApp {fqdn} at{" "}
                  <a
                    href="https://app.medispeak.in"
                    target="_blank"
                    rel="noreferrer"
            ...
            <div className="tw-flex tw-justify-center tw-items-center tw-p-4">
              <span>Loading Data from your MediSpeak Account</span>
            </div>
          )
        ) : (
      ...
      <div className="tw-flex tw-flex-col">
        ...
        {recordingUri && (
          <>
            ...
            <audio
              src={recordingUri}
              controls
              className="tw-w-full tw-p-2 tw-border-0 tw-text-sm focus:tw-outline-none tw-text-gray-700 tw-rounded-none tw-resize-none"
            />
          </>
        )}
        ...
        {transcription && (
          <>
            ...
            <p className="tw-w-full tw-p-2 tw-border-0 tw-text-sm focus:tw-outline-none tw-bg-gray-50 tw-text-gray-700 tw-rounded-none tw-resize-none">
        ...
        <div
          className="tw-flex tw-justify-between tw-items-center tw-mx-4 tw-my-2"
          ref={autofillRef}
        >
          {transcription && !transcriptionRecord ? (
            ...
            <div className="tw-flex tw-flex-col">
              ...
              <div className="tw-flex tw-flex-col tw-text-sm tw-text-gray-700">
                ...
                <span>Please wait, it might take a minute or two</span>
              </div>
            </div>
          ) : (
            <></>
          ...
          {transcription &&
            (autofillData ? (
              ...
              <div className="tw-button tw-rounded-full tw-flex tw-items-center tw-space-x-4 tw-px-4 tw-py-2 tw-bg-gray-100 tw-text-gray-700 tw-cursor-wait">
                <svg
                  className="tw-animate-spin -tw-ml-1 tw-mr-3 tw-h-5 tw-w-5 tw-text-gray-700"
      ...
      >
        <span className="tw-p-1 tw-rounded-full tw-bg-gray-100">
          ...
          >
            <path
              fillRule="evenodd"
              d="M10 0a10 10 0 1 0 0 20 10 10 0 0 0 0-20zm0 18.75a8.75 8.75 0 1 1 0-17.5 8.75 8.75 0 0 1 0 17.5zm-.625-5.625h1.25v-5h-1.25v5z"
              clipRule="evenodd"
            />
          </svg>
        </span>
